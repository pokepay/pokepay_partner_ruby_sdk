# CPM決済仕様
CPM決済はエンドユーザがコードを提示するタイプの取引形態である。エンドユーザ側、店舗ユーザのそれぞれからメタデータの入力を受け付けるタイミングがある。

## CPM決済の流れ
- エンドユーザがBankAPI(`POST /accounts/:uuid/cpm`)と通信しCPMトークンを生成
  - この際エンドユーザアプリは外部IDやアプリ内の状態などをメタデータとして付けることができ、店舗はAPIからそれを参照できる(BankAPI: `GET /cpm/:cpm_token`, PartnerAPI: `GetCpmToken`)。メタデータは全てのキーと値のペアが文字列であるようなフラットな構造のJSONオブジェクトで指定する
- エンドユーザは22桁のCPMトークンをスマホ等にバーコードや二次元コードとして表示する
- それを店舗ユーザが読み取り解析し、22桁のCPMトークンを得る
- 店舗ユーザがCPMトークンをつけて決済API(Bank APIの`POST /transactions`やPartner APIの`CreateCpmTransaction`、`CreateCpmExternalTransactionEvent`)を呼ぶことにより取引が完了する
  - この際店舗は自店舗のID、取引金額に加え、商品情報などのメタデータを付けることができる。これらは取引情報と紐付けて保存される。メタデータは全てのキーと値のペアが文字列であるようなフラットな構造のJSONオブジェクトで指定する
  - CPMトークンはCPM決済が完了すると失効する。1つのCPMトークンで作れる取引は1つである
  - CPMトークンを付けて決済APIを呼べるのは店舗ユーザのみである
  - 羃等性の担保のために、決済APIにはオプショナルなパラメータとして`request_id`を付けることができる
- エンドユーザ / 店舗ユーザはAPI(`GET /cpm/:cpm_token`)を通して任意のタイミングで取引の状態を取得できる
...
- その次4桁はBase64文字列で、マネーのUUIDの先頭3byte(16進表記で6桁)。例: `u48I` → マネーUUIDの冒頭6桁 `bb8f08` <!-- (loop for oct across (jose/base64:base64url-decode "u48I") do (format t "~2,'0x" oct)) -->
...
- Response
  ...
  - `description`: 取引説明文。最大200字の文字列
  - `type`: 取引種別
  - `is_modified`: キャンセルされているかどうか(boolean)
  - `products`: 商品情報 (new)
...
response = $client.send(Pokepay::Request::CreateCpmTransaction.new(
                          "90000022u48IXXXXXXXX",                       # cpm_token
...
#### 決済APIのrequest_idによる結果取得用API (`GET /transactions/results/{request_id}`, `GetTransactionResult`)
`CreateCpmTransaction`でレスポンスを喪失してしまったとして、`request_id`から取引結果を取得するためのAPI。取引結果が存在すればその内容が返り、存在しなければ`404 Not found`が返る。

- Request
  - Path parameter
    - `request_id`: リクエスト時に指定したrequest_id
    
Rubyコード例
```ruby
response = $client.send(Pokepay::Request::GetTransactionResult.new("54a99bb6-d926-4fea-8c56-a7df2d8c1135"))
```
...
- Request
  ...
  - Body parameter
    ...
    - `products`: キャンセル対象の商品情報。以下の内容からなるJSONオブジェクトの配列
      - `jan_code`: JANコード。64字以下の文字列
      - `name`: 商品名。256字以下の文字列
      - `unit_price`: 商品単価。0以上の数値
      - `price`: 全体の金額(例: 商品単価 × 個数)。0以上の数値
...
#### ポケペイ外部取引イベントAPI(CPM)のrequest_idによる結果取得用API (`GET /events/external-transactions/results/{request_id}`, `GetExternalTransactionResult`)
`CreateCpmExternalTransactionEvent`でレスポンスを喪失してしまったとして、`request_id`から取引結果を取得するためのAPI。取引結果が存在すればその内容が返り、存在しなければ`404 Not found`が返る。
...
Rubyコード例
```ruby
response = $client.send(Pokepay::Request::GetExternalTransactionResult.new("54a99bb6-d926-4fea-8c56-a7df2d8c1135"))
```
...
Rubyコード例
```ruby
response = $client.send(Pokepay::Request::GetCpmToken.new("90000022u48IXXXXXXXX"))
```
