...
response = $client.send(Pokepay::Request::ListCampaigns.new(
                          ...
                          available_from: "2023-04-11T18:17:56.000000Z",        # 指定された日時以降に適用可能期間が含まれているか
                          available_to: "2020-04-23T18:23:56.000000Z",          # 指定された日時以前に適用可能期間が含まれているか
                          page: 1,                                              # ページ番号
...
response = $client.send(Pokepay::Request::CreateCampaign.new(
                          "Hi2vDKbmu86aUF4jypKaAY4yQaiw0JpUpNfjrUKaUCU4cuncfOgZgC0vnz9vdHX3zI21M9POKUqkrXtAeLmERqX5bwDROtzb2hizqeaCyQXA4kt1s5IzgftNOCeiOWbpouk4VaYSYsKX6oU3L46cfTNsJ74FdhPrGorQztiuURWZ5r1OnryKkdpmMzmoITgipjScgSjEKEvn9t", # name: キャンペーン名
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          "2024-04-08T03:11:39.000000Z",                        # starts_at: キャンペーン開始日時
                          "2020-01-16T17:51:37.000000Z",                        # ends_at: キャンペーン終了日時
                          426,                                                  # priority: キャンペーンの適用優先度
                          "external-transaction",                               # event: イベント種別
                          bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント負担先店舗ID
                          description: "sfEeEirDJBvMOLUpWvpkfaBwAHAugbJ1Kgm",   # キャンペーンの説明文
                          status: "enabled",                                    # キャンペーン作成時の状態
                          point_expires_at: "2020-12-16T09:03:05.000000Z",      # ポイント有効期限(絶対日時指定)
                          point_expires_in_days: 9582,                          # ポイント有効期限(相対日数指定)
  ...
  "classification_code": "c123"
                          ...
                          applicable_days_of_week: [3, 3, 3],                   # キャンペーンを適用する曜日 (複数指定)
  ...
  "to": "23:59"
                          ...
                          minimum_number_of_products: 1756,                     # キャンペーンを適用する1会計内の商品個数の下限
                          minimum_number_of_amount: 7282,                       # キャンペーンを適用する1会計内の商品総額の下限
                          minimum_number_for_combination_purchase: 5418,        # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups: false,                  # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount: 8403,                               # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount: 9673,                         # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  "value": "bronze"
                          ...
                          budget_caps_amount: 1822639636                        # キャンペーン予算上限
))
```
...
response = $client.send(Pokepay::Request::UpdateCampaign.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # campaign_id: キャンペーンID
                          name: "6a6zzUqrHdosHdbmLywqukvEUDGTtuu5mLHhGQ9yekqoyNLKN2h7BNq3rRMob2yqEgXsKX0DNjA5LloLW2ZGwTADg0EGo2tY0BvAArU4c3Hcr3rYtMZs1YhEQlphw1DkmThPoIdPA7X1r8JTPyIk7mw82VAIRkHcNMgqN77FQwuiGtQW4p", # キャンペーン名
                          starts_at: "2020-10-01T17:26:30.000000Z",             # キャンペーン開始日時
                          ends_at: "2020-01-28T02:43:31.000000Z",               # キャンペーン終了日時
                          priority: 107,                                        # キャンペーンの適用優先度
                          event: "topup",                                       # イベント種別
                          description: "fz0ZAYuHKErS89ga",                      # キャンペーンの説明文
                          status: "enabled",                                    # キャンペーン作成時の状態
                          point_expires_at: "2020-11-10T02:44:02.000000Z",      # ポイント有効期限(絶対日時指定)
                          point_expires_in_days: 7933,                          # ポイント有効期限(相対日数指定)
  ...
  "to": "23:59"
                          ...
                          applicable_shop_ids: ["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], # キャンペーン適用対象となる店舗IDのリスト
                          blacklisted_shop_ids: ["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], # キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
                          minimum_number_of_products: 7673,                     # キャンペーンを適用する1会計内の商品個数の下限
                          minimum_number_of_amount: 4876,                       # キャンペーンを適用する1会計内の商品総額の下限
                          minimum_number_for_combination_purchase: 4213,        # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups: false,                  # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount: 1458,                               # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount: 2734,                         # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
                          applicable_account_metadata: {
  "key": "sex",
  "value": "male"
},                                                                              # ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
  ...
  "value": "bronze"
                          ...
                          budget_caps_amount: 1483666366                        # キャンペーン予算上限
))
```
