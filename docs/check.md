...
response = $client.send(Pokepay::Request::ListChecks.new(
                          ...
                          organization_code: "OO42l0o0g8SXRzZ3",                # 組織コード
                          expires_from: "2021-01-27T21:51:44.000000Z",          # 有効期限の期間によるフィルター(開始時点)
                          expires_to: "2023-03-15T18:08:06.000000Z",            # 有効期限の期間によるフィルター(終了時点)
                          created_from: "2021-07-22T01:16:05.000000Z",          # 作成日時の期間によるフィルター(開始時点)
                          created_to: "2023-09-02T10:58:19.000000Z",            # 作成日時の期間によるフィルター(終了時点)
                          issuer_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 発行店舗ID
                          description: "HeXuuwg12",                             # チャージQRコードの説明文
...
response = $client.send(Pokepay::Request::CreateCheck.new(
                          ...
                          money_amount: 7047.0,                                 # 付与マネー額
                          point_amount: 9319.0,                                 # 付与ポイント額
                          description: "test check",                            # 説明文(アプリ上で取引の説明文として表示される)
                          is_onetime: false,                                    # ワンタイムかどうかのフラグ
                          usage_limit: 6317,                                    # ワンタイムでない場合の最大読み取り回数
                          expires_at: "2021-01-19T04:03:13.000000Z",            # チャージQRコード自体の失効日時
                          point_expires_at: "2024-05-24T01:14:41.000000Z",      # チャージQRコードによって付与されるポイント残高の有効期限
...
response = $client.send(Pokepay::Request::UpdateCheck.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # check_id: チャージQRコードのID
                          money_amount: 3204.0,                                 # 付与マネー額
                          point_amount: 3777.0,                                 # 付与ポイント額
                          description: "test check",                            # チャージQRコードの説明文
                          is_onetime: false,                                    # ワンタイムかどうかのフラグ
                          usage_limit: 2235,                                    # ワンタイムでない場合の最大読み取り回数
                          expires_at: "2022-03-20T09:50:12.000000Z",            # チャージQRコード自体の失効日時
                          point_expires_at: "2023-07-21T00:03:27.000000Z",      # チャージQRコードによって付与されるポイント残高の有効期限
                          point_expires_in_days: 60,                            # チャージQRコードによって付与されるポイント残高の有効期限(相対日数指定)
                          bear_point_account: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント額を負担する店舗のウォレットID
