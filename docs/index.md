...
response = $client.send(Pokepay::Request::GetCpmToken.new(
                          "MKfAe1eK6cqTaSdyEQ3JdI"                              # cpm_token: CPMトークン
))
```
...
response = $client.send(Pokepay::Request::ListTransactions.new(
                          from: "2022-04-18T08:57:31.000000+09:00",             # 開始日時
                          to: "2020-12-10T01:01:54.000000+09:00",               # 終了日時
                          page: 1,                                              # ページ番号
...
response = $client.send(Pokepay::Request::CreateTransaction.new(
                          ...
                          money_amount: 7009,
                          point_amount: 8431,
                          point_expires_at: "2022-04-09T21:18:36.000000+09:00", # ポイント有効期限
                          description: "y"
))
```
...
response = $client.send(Pokepay::Request::ListTransactionsV2.new(
                          ...
                          customer_name: "太郎",                                  # エンドユーザー名
                          description: "店頭QRコードによる支払い",                         # 取引説明文
                          transaction_id: "Y8jDW2wIFN",                         # 取引ID
                          is_modified: false,                                   # キャンセルフラグ
                          types: ["topup", "payment"],                          # 取引種別 (複数指定可)、チャージ=topup、支払い=payment
                          from: "2023-07-17T11:22:24.000000+09:00",             # 開始日時
                          to: "2021-07-10T08:02:12.000000+09:00",               # 終了日時
...
response = $client.send(Pokepay::Request::CreateTopupTransaction.new(
                          ...
                          money_amount: 6492,                                   # マネー額
                          point_amount: 1192,                                   # ポイント額
                          point_expires_at: "2021-02-23T08:14:57.000000+09:00", # ポイント有効期限
...
response = $client.send(Pokepay::Request::CreateCpmTransaction.new(
                          "uFv74wv7ZzB6R2SZi7axn5",                             # cpm_token: CPMトークン
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # shop_id: 店舗ID
                          4330.0,                                               # amount: 取引金額
...
response = $client.send(Pokepay::Request::CreateExchangeTransaction.new(
                          ...
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          493,
                          description: "rdaRF3PfQBnHpApnhsgTJtt9b0SHd58KGyLYRCmOt7eLraOE0wRjhg3kItV3jADGageeyHKOwECoIGZU5j3HQU2LPvbvDHGM6LNFD9xhu0Fk5q12hP5",
                          request_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"    # リクエストID
))
```
...
response = $client.send(Pokepay::Request::RefundTransaction.new(
                          ...
                          returning_point_expires_at: "2021-11-09T11:48:33.000000+09:00" # 返却ポイントの有効期限
))
```
...
response = $client.send(Pokepay::Request::GetAccountTransferSummary.new(
                          ...
                          from: "2022-06-01T04:23:34.000000+09:00",             # 集計期間の開始時刻
                          to: "2020-06-24T02:20:50.000000+09:00",               # 集計期間の終了時刻
...
response = $client.send(Pokepay::Request::ListTransfers.new(
                          from: "2020-04-18T18:34:23.000000+09:00",
                          to: "2022-06-28T22:05:58.000000+09:00",
                          page: 8457,
                          per_page: 4763,
                          shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          shop_name: "6VyoN0alDoUQf5IzyN3d0qREqEQ1iA54NdX3mhkRF1J9HaH2Li9lRmoxFQKsnLaLPglUwzQEdMsqFxnq4xC9FTo71hLvY3AjA2nifwlTd4uftbURvJpAIFdsy6qRJi",
                          customer_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          customer_name: "kh963ixEOU8vlpHAesWABV81UfaRGD",
                          transaction_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          is_modified: false,
                          transaction_types: ["exchange", "expire", "transfer"],
                          transfer_types: ["transfer", "payment", "exchange", "topup", "expire", "cashback"], # 取引明細の種類でフィルターします。
...
response = $client.send(Pokepay::Request::ListTransfersV2.new(
                          shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",      # 店舗ID
                          shop_name: "4QFhMHT3Kfg2zQVWNCs1Qp9NbWahfX3IlE4osXdHrvjBAuuG847LOlvGyoE1VeUKTOQICKY9Vm3hz2kE2fuYpSazfY2iVzUITZXJqGCQzG5vgcFWCnOCuMzhG2UByAEYJjmnIjQe0RRZeii9RiBsWAU1POZerxi4yVgC3DLJXU7OcLNBL8E5ZFfjFnVMbltHeFRWSnoqc1L0CxETFiBWrZsD0D8eKFs1tot7y2okcefbc2jYVeVGNfa", # 店舗名
                          customer_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",  # エンドユーザーID
                          customer_name: "F1e5yToinw9aop7xdnRoOktIwFrjXVxpnweBnMnGDaEFssXp6TWSngdAZpL1lbFl30lt8PE9rRZhwqiE5pziP8FEhG5s1lcU1i26ObO0SxJa1bFQUUDsvsrmLVJx5kyo3bTrHmhOyh9HftvyRtN4Cr8rRNqt0prNATlUYTfGtCeGENB715jPqh4zDUpSgB", # エンドユーザー名
                          transaction_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 取引ID
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          is_modified: false,                                   # キャンセルフラグ
                          transaction_types: ["exchange", "expire", "cashback", "payment"], # 取引種別 (複数指定可)、チャージ=topup、支払い=payment
                          ...
                          per_page: 50,                                         # 1ページ分の取引数
                          transfer_types: ["cashback", "topup", "campaign", "expire", "exchange", "payment"], # 取引明細種別 (複数指定可)
                          description: "店頭QRコードによる支払い",                         # 取引詳細説明文
                          from: "2022-12-17T06:38:14.000000+09:00",             # 開始日時
                          to: "2020-04-28T00:12:43.000000+09:00"                # 終了日時
))
```
...
response = $client.send(Pokepay::Request::CreateCheck.new(
                          ...
                          money_amount: 9780.0,                                 # 付与マネー額
                          point_amount: 8277.0,                                 # 付与ポイント額
                          description: "test check",                            # 説明文(アプリ上で取引の説明文として表示される)
                          is_onetime: false,                                    # ワンタイムかどうかのフラグ
                          usage_limit: 9779,                                    # ワンタイムでない場合の最大読み取り回数
                          expires_at: "2020-01-29T05:47:33.000000+09:00",       # チャージQRコード自体の失効日時
                          point_expires_at: "2020-07-20T17:09:44.000000+09:00", # チャージQRコードによって付与されるポイント残高の有効期限
...
response = $client.send(Pokepay::Request::ListChecks.new(
                          ...
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          organization_code: "gBeZqxKIp3Gq2cx6u59m45TAfwJ",     # 組織コード
                          expires_from: "2020-05-30T14:16:41.000000+09:00",     # 有効期限の期間によるフィルター(開始時点)
                          expires_to: "2021-04-15T21:26:59.000000+09:00",       # 有効期限の期間によるフィルター(終了時点)
                          created_from: "2020-04-14T19:15:40.000000+09:00",     # 作成日時の期間によるフィルター(開始時点)
                          created_to: "2020-10-19T17:45:22.000000+09:00",       # 作成日時の期間によるフィルター(終了時点)
                          issuer_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 発行店舗ID
                          description: "rJg",                                   # チャージQRコードの説明文
...
response = $client.send(Pokepay::Request::UpdateCheck.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # check_id: チャージQRコードのID
                          money_amount: 5520.0,                                 # 付与マネー額
                          point_amount: 7522.0,                                 # 付与ポイント額
                          description: "test check",                            # チャージQRコードの説明文
                          is_onetime: false,                                    # ワンタイムかどうかのフラグ
                          usage_limit: 7357,                                    # ワンタイムでない場合の最大読み取り回数
                          expires_at: "2023-11-30T12:48:00.000000+09:00",       # チャージQRコード自体の失効日時
                          point_expires_at: "2021-10-05T19:47:37.000000+09:00", # チャージQRコードによって付与されるポイント残高の有効期限
                          point_expires_in_days: 60,                            # チャージQRコードによって付与されるポイント残高の有効期限(相対日数指定)
                          bear_point_account: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント額を負担する店舗のウォレットID
...
response = $client.send(Pokepay::Request::ListBills.new(
                          page: 4289,                                           # ページ番号
                          per_page: 1293,                                       # 1ページの表示数
                          bill_id: "oD",                                        # 支払いQRコードのID
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          organization_code: "2rbUVoc-q-i-XqJ-l75jDF-y5",       # 組織コード
                          description: "test bill",                             # 取引説明文
                          created_from: "2020-08-10T10:30:22.000000+09:00",     # 作成日時(起点)
                          created_to: "2020-12-01T09:07:20.000000+09:00",       # 作成日時(終点)
                          shop_name: "bill test shop1",                         # 店舗名
                          shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",      # 店舗ID
                          lower_limit_amount: 4868,                             # 金額の範囲によるフィルタ(下限)
                          upper_limit_amount: 1194,                             # 金額の範囲によるフィルタ(上限)
...
response = $client.send(Pokepay::Request::ListAccountBalances.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # account_id: ウォレットID
                          page: 7638,                                           # ページ番号
                          per_page: 3012,                                       # 1ページ分の取引数
                          expires_at_from: "2022-02-18T18:39:33.000000+09:00",  # 有効期限の期間によるフィルター(開始時点)
                          expires_at_to: "2020-04-10T02:39:36.000000+09:00",    # 有効期限の期間によるフィルター(終了時点)
...
response = $client.send(Pokepay::Request::ListAccountExpiredBalances.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # account_id: ウォレットID
                          page: 1531,                                           # ページ番号
                          per_page: 1428,                                       # 1ページ分の取引数
                          expires_at_from: "2022-06-25T14:41:00.000000+09:00",  # 有効期限の期間によるフィルター(開始時点)
                          expires_at_to: "2020-10-17T01:50:42.000000+09:00",    # 有効期限の期間によるフィルター(終了時点)
...
response = $client.send(Pokepay::Request::UpdateCustomerAccount.new(
                          ...
                          account_name: "WGKZOqV5ZHP5SzalB7kDHNOTEfJ",          # アカウント名
                          external_id: "CmTDiP3dCYUB19JAPgCAWJJQoQVP5xPnmC8ttUiz", # 外部ID
...
response = $client.send(Pokepay::Request::GetCustomerAccounts.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          page: 9464,                                           # ページ番号
                          per_page: 4663,                                       # 1ページ分のウォレット数
                          created_at_from: "2022-11-28T06:00:35.000000+09:00",  # ウォレット作成日によるフィルター(開始時点)
                          created_at_to: "2023-10-05T14:49:10.000000+09:00",    # ウォレット作成日によるフィルター(終了時点)
                          is_suspended: true,                                   # ウォレットが凍結状態かどうかでフィルターする
                          status: "pre-closed",                                 # ウォレット状態
                          external_id: "fly1cPIpj3JRPzCeiFBuP3fboz",            # 外部ID
                          tel: "03-03-4821",                                    # エンドユーザーの電話番号
                          email: "3bt6HBBjf7@6dwE.com"                          # エンドユーザーのメールアドレス
))
```
...
response = $client.send(Pokepay::Request::CreateCustomerAccount.new(
                          ...
                          external_id: "RetPYVvMOieUVr"                         # 外部ID
))
```
...
response = $client.send(Pokepay::Request::GetShopAccounts.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          page: 6824,                                           # ページ番号
                          per_page: 6556,                                       # 1ページ分のウォレット数
                          created_at_from: "2021-06-11T23:57:25.000000+09:00",  # ウォレット作成日によるフィルター(開始時点)
                          created_at_to: "2023-12-28T14:07:28.000000+09:00",    # ウォレット作成日によるフィルター(終了時点)
                          is_suspended: true                                    # ウォレットが凍結状態かどうかでフィルターする
))
```
...
response = $client.send(Pokepay::Request::ListCustomerTransactions.new(
                          ...
                          from: "2021-02-15T08:20:02.000000+09:00",             # 開始日時
                          to: "2020-12-10T15:53:43.000000+09:00",               # 終了日時
                          page: 1,                                              # ページ番号
                          per_page: 50                                          # 1ページ分の取引数
))
```
...
response = $client.send(Pokepay::Request::ListShops.new(
                          ...
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          name: "oxスーパー三田店",                                    # 店舗名
                          postal_code: "993-2237",                              # 店舗の郵便番号
                          address: "東京都港区芝...",                                 # 店舗の住所
                          tel: "006378-8383",                                   # 店舗の電話番号
                          email: "hNr0yJILOb@CBfy.com",                         # 店舗のメールアドレス
                          external_id: "wcVkUfQaz",                             # 店舗の外部ID
...
response = $client.send(Pokepay::Request::CreateShop.new(
                          ...
                          shop_postal_code: "954-3609",                         # 店舗の郵便番号
                          shop_address: "東京都港区芝...",                            # 店舗の住所
                          shop_tel: "053-530-403",                              # 店舗の電話番号
                          shop_email: "7fZazhzo2r@VyuT.com",                    # 店舗のメールアドレス
                          shop_external_id: "t0YgSvCqiNZ",                      # 店舗の外部ID
...
response = $client.send(Pokepay::Request::CreateShopV2.new(
                          "oxスーパー三田店",                                          # name: 店舗名
                          postal_code: "9651044",                               # 店舗の郵便番号
                          address: "東京都港区芝...",                                 # 店舗の住所
                          tel: "031-444-3141",                                  # 店舗の電話番号
                          email: "Fv75Yj76dO@8Byp.com",                         # 店舗のメールアドレス
                          external_id: "JvkQXVQsy3djpI6OKsW",                   # 店舗の外部ID
                          organization_code: "ox-supermarket",                  # 組織コード
...
response = $client.send(Pokepay::Request::UpdateShop.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # shop_id: 店舗ユーザーID
                          name: "oxスーパー三田店",                                    # 店舗名
                          postal_code: "4105750",                               # 店舗の郵便番号
                          address: "東京都港区芝...",                                 # 店舗の住所
                          tel: "0310-581-5867",                                 # 店舗の電話番号
                          email: "rhQOmjEJqN@bQRY.com",                         # 店舗のメールアドレス
                          external_id: "biJ3sraEQwubZAzZK7SvDcKs",              # 店舗の外部ID
...
response = $client.send(Pokepay::Request::CreateUserAccount.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # user_id: ユーザーID
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          name: "EoC0drgZxPMynqzwaKWuGCi0r66sFnb4EHhtpyHhydYr", # ウォレット名
                          external_id: "vl0kK9EZIvCFy",                         # 外部ID
                          metadata: "{\"key1\":\"foo\",\"key2\":\"bar\"}"       # ウォレットに付加するメタデータ
))
```
...
response = $client.send(Pokepay::Request::GetPrivateMoneyOrganizationSummaries.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          from: "2023-06-07T16:20:30.000000+09:00",             # 開始日時(toと同時に指定する必要有)
                          to: "2022-09-28T23:08:23.000000+09:00",               # 終了日時(fromと同時に指定する必要有)
                          page: 1,                                              # ページ番号
                          per_page: 50                                          # 1ページ分の取引数
))
```
...
response = $client.send(Pokepay::Request::GetPrivateMoneySummary.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          from: "2021-11-14T01:39:13.000000+09:00",             # 開始日時
                          to: "2020-11-28T22:19:57.000000+09:00"                # 終了日時
))
```
...
response = $client.send(Pokepay::Request::BulkCreateTransaction.new(
                          "8w7WgJYTO6su6qoaMRZmasHrk",                          # name: 一括取引タスク名
                          "9vwkA7HyPd",                                         # content: 取引する情報のCSV
                          "nBLklXsIbPVWFWz9qTrxNXvnjmpY5dLg1Arq",               # request_id: リクエストID
                          description: "P",                                     # 一括取引の説明
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" # マネーID
))
```
...
response = $client.send(Pokepay::Request::CreateCampaign.new(
                          "L5y9OwEZL6upsEahEEc9ppcYhMmTxGmFhPaLpum9C5akJe4VZRN7pOcxJycIkp3QiVAW0CZ4fK4wIriLXY7EWHeBz2kXjOJRloaGHqDMVjypNWifc5GoqwK", # name: キャンペーン名
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          "2022-09-10T10:28:00.000000+09:00",                   # starts_at: キャンペーン開始日時
                          "2022-09-02T09:35:43.000000+09:00",                   # ends_at: キャンペーン終了日時
                          6280,                                                 # priority: キャンペーンの適用優先度
                          "external-transaction",                               # event: イベント種別
                          bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント負担先店舗ID
                          description: "C90qeAtolOJthqJlGPYyzrdgyMTfPbTeRjlXfdNzBimf2h8leoKYGWUrBLmE9tHzH", # キャンペーンの説明文
                          status: "disabled",                                   # キャンペーン作成時の状態
                          point_expires_at: "2023-02-14T23:34:24.000000+09:00", # ポイント有効期限(絶対日時指定)
                          point_expires_in_days: 8693,                          # ポイント有効期限(相対日数指定)
  ...
  "to": "23:59"
                          ...
                          minimum_number_for_combination_purchase: 4852,        # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups: false,                  # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount: 3114,                               # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount: 308,                          # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  "value": "male"
                          ...
                          budget_caps_amount: 12784478                          # キャンペーン予算上限
))
```
...
response = $client.send(Pokepay::Request::ListCampaigns.new(
                          ...
                          available_from: "2023-05-08T14:14:30.000000+09:00",   # 指定された日時以降に適用可能期間が含まれているか
                          available_to: "2020-09-25T19:49:03.000000+09:00",     # 指定された日時以前に適用可能期間が含まれているか
                          page: 1,                                              # ページ番号
...
response = $client.send(Pokepay::Request::UpdateCampaign.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # campaign_id: キャンペーンID
                          name: "P256HghUwetr29bx",                             # キャンペーン名
                          starts_at: "2024-01-12T22:59:11.000000+09:00",        # キャンペーン開始日時
                          ends_at: "2021-08-19T18:56:02.000000+09:00",          # キャンペーン終了日時
                          priority: 2335,                                       # キャンペーンの適用優先度
                          event: "payment",                                     # イベント種別
                          description: "s31TlnHtc74T9nqzSWeYIT4SuYpkOpB6pSmy0suDrkkNa3QGtQIAy2rhdqjhcaGUaLUKsybg2aEINLo78ODsjNXUGeNtNLkTz", # キャンペーンの説明文
                          status: "enabled",                                    # キャンペーン作成時の状態
                          point_expires_at: "2022-11-17T21:24:53.000000+09:00", # ポイント有効期限(絶対日時指定)
                          point_expires_in_days: 5401,                          # ポイント有効期限(相対日数指定)
                          is_exclusive: true,                                   # キャンペーンの重複設定
                          subject: "money",                                     # ポイント付与の対象金額の種別
                          amount_based_point_rules: [{
  "point_amount": 5,
  ...
  "required_count": 2
                          ...
                          applicable_days_of_week: [0, 3, 6],                   # キャンペーンを適用する曜日 (複数指定)
                          applicable_time_ranges: [{
  "from": "12:00",
  "to": "23:59"
}],                                                                             # キャンペーンを適用する時間帯 (複数指定)
                          applicable_shop_ids: ["xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"], # キャンペーン適用対象となる店舗IDのリスト
                          minimum_number_for_combination_purchase: 9910,        # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups: true,                   # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount: 2123,                               # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount: 9841,                         # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
                          applicable_account_metadata: {
  "key": "sex",
  "value": "male"
},                                                                              # ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
                          budget_caps_amount: 1114663244                        # キャンペーン予算上限
))
```
...
response = $client.send(Pokepay::Request::ListCoupons.new(
                          ...
                          coupon_id: "f",                                       # クーポンID
                          coupon_name: "sm",                                    # クーポン名
                          issued_shop_name: "jGy2XL89",                         # 発行店舗名
                          available_shop_name: "zTUj",                          # 利用可能店舗名
                          available_from: "2023-01-19T06:20:10.000000+09:00",   # 利用可能期間 (開始日時)
                          available_to: "2020-11-15T18:16:09.000000+09:00",     # 利用可能期間 (終了日時)
                          page: 1,                                              # ページ番号
                          per_page: 50                                          # 1ページ分の取得数
))
```
...
response = $client.send(Pokepay::Request::CreateCoupon.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          "Z1CL1ozTXiFpvPcP4seURVjkKZsf0y2TWdCS4SATijReOtNnR872lolIx9QVcnAbyreqNbB9fkzFltAaCnmZFopgefxIacQ1AyQrSPDO1MM35WEy8yV",
                          "2020-08-05T07:38:08.000000+09:00",
                          "2023-03-15T17:42:01.000000+09:00",
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # issued_shop_id: 発行元の店舗ID
                          description: "gpvbXeoOQia5qbbDQVlHIzOE7wrLcPtiesvO6HBa",
                          discount_amount: 3420,
                          discount_percentage: 4333.0,
                          discount_upper_limit: 3571,
                          display_starts_at: "2022-11-22T07:40:46.000000+09:00", # クーポンの掲載期間(開始日時)
                          display_ends_at: "2020-03-29T06:02:35.000000+09:00",  # クーポンの掲載期間(終了日時)
                          is_disabled: true,                                    # 無効化フラグ
                          is_hidden: false,                                     # クーポン一覧に掲載されるかどうか
                          is_public: false,                                     # アプリ配信なしで受け取れるかどうか
                          code: "MOLgQCuJHM",                                   # クーポン受け取りコード
                          usage_limit: 610,                                     # ユーザごとの利用可能回数(NULLの場合は無制限)
                          min_amount: 6197,                                     # クーポン適用可能な最小取引額
...
response = $client.send(Pokepay::Request::UpdateCoupon.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # coupon_id: クーポンID
                          name: "94stM1FLlWrur5uNkACOO3BotDxqlluxYKWpOtfmUzi0dZfpLKzKjfcb4HAAh1mdWusGLMSR",
                          description: "SUD0sAddhJiEvsCvYpluLDJm3ul08w8Ie6eJyFK46JFllRfuMr9FUmav4Lda3TOmdveMFwIXmq5gtPoAqpVuAq7iRsiSEhlLoK6ksxhBzd5KJrvYdK4fwu3YwJeLwqiF2DxpUMIOfTSnLTRTnMKY68",
                          discount_amount: 1935,
                          discount_percentage: 3185.0,
                          discount_upper_limit: 5425,
                          starts_at: "2021-10-06T14:14:01.000000+09:00",
                          ends_at: "2022-08-27T06:03:07.000000+09:00",
                          display_starts_at: "2020-08-05T10:57:03.000000+09:00", # クーポンの掲載期間(開始日時)
                          display_ends_at: "2023-11-16T01:14:29.000000+09:00",  # クーポンの掲載期間(終了日時)
                          is_disabled: false,                                   # 無効化フラグ
                          is_hidden: true,                                      # クーポン一覧に掲載されるかどうか
                          is_public: false,                                     # アプリ配信なしで受け取れるかどうか
                          code: "btphepeKN",                                    # クーポン受け取りコード
                          usage_limit: 8917,                                    # ユーザごとの利用可能回数(NULLの場合は無制限)
                          min_amount: 5625,                                     # クーポン適用可能な最小取引額
                          is_shop_specified: false,                             # 特定店舗限定のクーポンかどうか
...
response = $client.send(Pokepay::Request::CreateBank.new(
                          ...
                          email: "iOQXvlaWuK@Q7VI.com",                         # ユーザーのメールアドレス
