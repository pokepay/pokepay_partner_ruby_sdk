...
response = $client.send(Pokepay::Request::GetCpmToken.new(
                          "pPOiMfbGf52VnNvuPlMemc"                              # cpm_token: CPMトークン
))
```
...
response = $client.send(Pokepay::Request::ListTransactions.new(
                          from: "2021-04-09T01:58:50.000000+09:00",             # 開始日時
                          to: "2023-08-22T16:52:22.000000+09:00",               # 終了日時
                          page: 1,                                              # ページ番号
...
response = $client.send(Pokepay::Request::CreateTransaction.new(
                          ...
                          money_amount: 1121,
                          point_amount: 174,
                          point_expires_at: "2023-05-06T11:00:57.000000+09:00", # ポイント有効期限
                          description: "FA3vbFwDGQw5KiUBSDLVRajof131GTwDeRVEvYAf5Vg8il9BfUy6zOVgCYfr1G3TuMY90tDfc5P5Chx1wi5CCNzE6iOkxqqgj4DAcO2oSKDPRr0tZ7YrhzQy6rfPUOOyvDQce0SU9y2sxnLDcRZNKnZKp9a0YEIEOYFT9CRoY3xHdVwAc860HBuEU4lCZjWXcy"
))
```
...
response = $client.send(Pokepay::Request::ListTransactionsV2.new(
                          ...
                          customer_name: "太郎",                                  # エンドユーザー名
                          description: "店頭QRコードによる支払い",                         # 取引説明文
                          transaction_id: "6",                                  # 取引ID
                          is_modified: true,                                    # キャンセルフラグ
                          types: ["topup", "payment"],                          # 取引種別 (複数指定可)、チャージ=topup、支払い=payment
                          from: "2020-05-23T12:10:32.000000+09:00",             # 開始日時
                          to: "2020-01-29T12:57:06.000000+09:00",               # 終了日時
...
response = $client.send(Pokepay::Request::CreateTopupTransaction.new(
                          ...
                          money_amount: 9962,                                   # マネー額
                          point_amount: 1181,                                   # ポイント額
                          point_expires_at: "2023-01-05T04:39:21.000000+09:00", # ポイント有効期限
...
response = $client.send(Pokepay::Request::CreateCpmTransaction.new(
                          "TUOfgOo2ImGFyITAOAqNUA",                             # cpm_token: CPMトークン
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # shop_id: 店舗ID
                          276.0,                                                # amount: 取引金額
...
response = $client.send(Pokepay::Request::CreateExchangeTransaction.new(
                          ...
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          7157,
                          description: "TquEfUXM8EFu0LJ2f0iMNxJE3ha3BQ5rf4nWvPefeFxEW8ghcv6gwQtCFX0HeiCeWCMgJb5M4eioS3MTG7Q7tjpzo7N8JOfwaimv2ip4iUt62CosGVgTSmvDC3Gdtw",
                          request_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"    # リクエストID
))
```
...
response = $client.send(Pokepay::Request::RefundTransaction.new(
                          ...
                          returning_point_expires_at: "2022-01-28T05:41:39.000000+09:00" # 返却ポイントの有効期限
))
```
...
response = $client.send(Pokepay::Request::GetAccountTransferSummary.new(
                          ...
                          from: "2020-01-02T18:35:03.000000+09:00",             # 集計期間の開始時刻
                          to: "2021-04-07T08:17:19.000000+09:00",               # 集計期間の終了時刻
...
response = $client.send(Pokepay::Request::ListTransfers.new(
                          from: "2022-08-04T03:01:54.000000+09:00",
                          to: "2020-12-06T12:29:49.000000+09:00",
                          page: 1860,
                          per_page: 4671,
                          shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          shop_name: "lcTQeiwvJz71bVKd59Vfo7R4nVFzev0LUcy9WBYGGU3zCNgCK6USyVL2kXVRiIas0GsUgfMMwEqkoItNjfRE3qwqi6FOkZARcJmMudVpgfwW76se5z3VJcxDOCiYRQKlQ8uQqvmATkLl18MRTmgt3HXYOOvwtw5L6zduKnCgvO7YYcbjDF77iopJuE1HikfYzM2pCfIHeKjhGpShOhtPS236CEssa4UdiuM4ZfhwmCwg",
                          customer_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          customer_name: "IjTWnNRaJkYfTWfcthVgveT7cKrTwlYLwWp7xsdr9W1WooQgNSLRwOXq958EOxd243GUyMlwVqFtZXjSqLB9hBomUF0DuNXI6h69mktiiw4cR6CeZOcEogeo8e3RqoyiWf9fTKI0IhM8uSDHDVOEPWNuWP5Mg0",
                          transaction_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          is_modified: false,
                          transaction_types: ["expire", "cashback", "transfer"],
                          transfer_types: ["exchange", "expire"],               # 取引明細の種類でフィルターします。
...
response = $client.send(Pokepay::Request::ListTransfersV2.new(
                          shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",      # 店舗ID
                          shop_name: "a2twnfCIcYNWH5fwoJsPbAf1GHe3XlqoX4mHpX2GcufUiibirEKSiReJmiJlMjZr5yAbIdO0HHuwTXvpJLVVbTpjY4hMtBV5H1Yv3Ciqznf9p0qlPWfkrdAjWVxnmcZsKpqFagdeqJoHl4C", # 店舗名
                          customer_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",  # エンドユーザーID
                          customer_name: "CWE2QBA3UEsDOn2xir9cPsFpoyzeta0X0C3gxENXdbfc3ukWlyORUQYrGiSW4VNwV", # エンドユーザー名
                          transaction_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 取引ID
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          is_modified: true,                                    # キャンセルフラグ
                          transaction_types: ["topup", "transfer"],             # 取引種別 (複数指定可)、チャージ=topup、支払い=payment
                          ...
                          per_page: 50,                                         # 1ページ分の取引数
                          transfer_types: ["expire", "exchange", "coupon", "payment", "cashback", "topup", "transfer", "campaign"], # 取引明細種別 (複数指定可)
                          description: "店頭QRコードによる支払い",                         # 取引詳細説明文
                          from: "2020-04-11T15:22:18.000000+09:00",             # 開始日時
                          to: "2021-01-25T00:39:40.000000+09:00"                # 終了日時
))
```
...
response = $client.send(Pokepay::Request::CreateCheck.new(
                          ...
                          money_amount: 4790.0,                                 # 付与マネー額
                          point_amount: 6207.0,                                 # 付与ポイント額
                          description: "test check",                            # 説明文(アプリ上で取引の説明文として表示される)
                          is_onetime: true,                                     # ワンタイムかどうかのフラグ
                          usage_limit: 7566,                                    # ワンタイムでない場合の最大読み取り回数
                          expires_at: "2020-10-08T08:22:48.000000+09:00",       # チャージQRコード自体の失効日時
                          point_expires_at: "2022-07-13T11:10:41.000000+09:00", # チャージQRコードによって付与されるポイント残高の有効期限
...
response = $client.send(Pokepay::Request::ListChecks.new(
                          ...
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          organization_code: "Us0HI5IbALIdlY931qMZCvyEkJo",     # 組織コード
                          expires_from: "2020-04-20T09:40:24.000000+09:00",     # 有効期限の期間によるフィルター(開始時点)
                          expires_to: "2022-04-08T08:44:52.000000+09:00",       # 有効期限の期間によるフィルター(終了時点)
                          created_from: "2023-01-20T10:51:12.000000+09:00",     # 作成日時の期間によるフィルター(開始時点)
                          created_to: "2022-10-21T22:34:58.000000+09:00",       # 作成日時の期間によるフィルター(終了時点)
                          issuer_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # 発行店舗ID
                          description: "wN41B",                                 # チャージQRコードの説明文
...
response = $client.send(Pokepay::Request::UpdateCheck.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # check_id: チャージQRコードのID
                          money_amount: 7446.0,                                 # 付与マネー額
                          point_amount: 4104.0,                                 # 付与ポイント額
                          description: "test check",                            # チャージQRコードの説明文
                          is_onetime: true,                                     # ワンタイムかどうかのフラグ
                          usage_limit: 660,                                     # ワンタイムでない場合の最大読み取り回数
                          expires_at: "2023-12-14T05:23:40.000000+09:00",       # チャージQRコード自体の失効日時
                          point_expires_at: "2020-01-19T10:10:39.000000+09:00", # チャージQRコードによって付与されるポイント残高の有効期限
                          point_expires_in_days: 60,                            # チャージQRコードによって付与されるポイント残高の有効期限(相対日数指定)
                          bear_point_account: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント額を負担する店舗のウォレットID
...
response = $client.send(Pokepay::Request::ListBills.new(
                          page: 410,                                            # ページ番号
                          per_page: 5199,                                       # 1ページの表示数
                          bill_id: "nfr5maIdxC",                                # 支払いQRコードのID
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          organization_code: "9o4a-5-E9x---80-oZo-",            # 組織コード
                          description: "test bill",                             # 取引説明文
                          created_from: "2023-08-06T17:31:37.000000+09:00",     # 作成日時(起点)
                          created_to: "2023-05-15T19:27:41.000000+09:00",       # 作成日時(終点)
                          shop_name: "bill test shop1",                         # 店舗名
                          shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",      # 店舗ID
                          lower_limit_amount: 9290,                             # 金額の範囲によるフィルタ(下限)
                          upper_limit_amount: 9035,                             # 金額の範囲によるフィルタ(上限)
...
response = $client.send(Pokepay::Request::ListAccountBalances.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # account_id: ウォレットID
                          page: 5764,                                           # ページ番号
                          per_page: 7,                                          # 1ページ分の取引数
                          expires_at_from: "2022-01-28T02:12:21.000000+09:00",  # 有効期限の期間によるフィルター(開始時点)
                          expires_at_to: "2022-02-14T18:33:30.000000+09:00",    # 有効期限の期間によるフィルター(終了時点)
...
response = $client.send(Pokepay::Request::ListAccountExpiredBalances.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # account_id: ウォレットID
                          page: 1986,                                           # ページ番号
                          per_page: 1514,                                       # 1ページ分の取引数
                          expires_at_from: "2020-11-20T12:11:58.000000+09:00",  # 有効期限の期間によるフィルター(開始時点)
                          expires_at_to: "2021-12-02T12:24:17.000000+09:00",    # 有効期限の期間によるフィルター(終了時点)
                          direction: "desc"                                     # 有効期限によるソート順序
))
```
...
response = $client.send(Pokepay::Request::UpdateCustomerAccount.new(
                          ...
                          account_name: "s0JQ5pXyK5DncfhRRDt8iHINRzVcHW5oEQLpTf8kGoFrxCNdyk3xmIJJ8jY2iwQlvXIritKzeZ9U5JeEBAv4wmmUncnpQdXVdxmtt3sRjnIpLVikvhD3DioLYmHxZM1Xqj2zsj4cbsotlI3P6U5O8Vq2q3MzTnRpygKhorJw6qclDZ9vzAhma7VNshFwPixFrSnT8qiIZl7wpYMxVgDkMtHA3CmcNj4x", # アカウント名
                          external_id: "KWcyvADxcDyTPyn",                       # 外部ID
...
response = $client.send(Pokepay::Request::GetCustomerAccounts.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          page: 9013,                                           # ページ番号
                          per_page: 1552,                                       # 1ページ分のウォレット数
                          created_at_from: "2021-06-06T14:37:24.000000+09:00",  # ウォレット作成日によるフィルター(開始時点)
                          created_at_to: "2020-08-15T10:35:10.000000+09:00",    # ウォレット作成日によるフィルター(終了時点)
                          is_suspended: true,                                   # ウォレットが凍結状態かどうかでフィルターする
                          status: "suspended",                                  # ウォレット状態
                          external_id: "0x7GaxpJnwNAF49BedKHAjJgasaJ",          # 外部ID
                          tel: "091-64-2547",                                   # エンドユーザーの電話番号
                          email: "pIrhEAHIMk@6Txi.com"                          # エンドユーザーのメールアドレス
))
```
...
response = $client.send(Pokepay::Request::CreateCustomerAccount.new(
                          ...
                          external_id: "nWWYlomI4xwdC4528NZTchhiv927YNdjFvp1v2pbrfQR6gMB" # 外部ID
))
```
...
response = $client.send(Pokepay::Request::GetShopAccounts.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          page: 7382,                                           # ページ番号
                          per_page: 36,                                         # 1ページ分のウォレット数
                          created_at_from: "2020-06-22T20:13:22.000000+09:00",  # ウォレット作成日によるフィルター(開始時点)
                          created_at_to: "2021-11-07T02:43:00.000000+09:00",    # ウォレット作成日によるフィルター(終了時点)
...
response = $client.send(Pokepay::Request::ListCustomerTransactions.new(
                          ...
                          from: "2020-02-11T15:47:22.000000+09:00",             # 開始日時
                          to: "2021-04-13T00:18:22.000000+09:00",               # 終了日時
                          page: 1,                                              # ページ番号
                          per_page: 50                                          # 1ページ分の取引数
))
```
...
response = $client.send(Pokepay::Request::ListShops.new(
                          ...
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # マネーID
                          name: "oxスーパー三田店",                                    # 店舗名
                          postal_code: "019-5100",                              # 店舗の郵便番号
                          address: "東京都港区芝...",                                 # 店舗の住所
                          tel: "07-6041-7089",                                  # 店舗の電話番号
                          email: "rKq4sJO6rB@LrEu.com",                         # 店舗のメールアドレス
                          external_id: "OIUhfmdoteaQeaxk",                      # 店舗の外部ID
...
response = $client.send(Pokepay::Request::CreateShop.new(
                          ...
                          shop_postal_code: "8299791",                          # 店舗の郵便番号
                          shop_address: "東京都港区芝...",                            # 店舗の住所
                          shop_tel: "064-273532",                               # 店舗の電話番号
                          shop_email: "aawLQI2iHs@cf1x.com",                    # 店舗のメールアドレス
                          shop_external_id: "BzvMXLPrKJFStYgmhsXrEvfyIu",       # 店舗の外部ID
...
response = $client.send(Pokepay::Request::CreateShopV2.new(
                          "oxスーパー三田店",                                          # name: 店舗名
                          postal_code: "8393470",                               # 店舗の郵便番号
                          address: "東京都港区芝...",                                 # 店舗の住所
                          tel: "0665-2171144",                                  # 店舗の電話番号
                          email: "VaJF7ByFIT@DRDh.com",                         # 店舗のメールアドレス
                          external_id: "1aYSFLzMxGyy3wRuF5QtrPJCxvo8k",         # 店舗の外部ID
                          organization_code: "ox-supermarket",                  # 組織コード
...
response = $client.send(Pokepay::Request::UpdateShop.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # shop_id: 店舗ユーザーID
                          name: "oxスーパー三田店",                                    # 店舗名
                          postal_code: "258-8638",                              # 店舗の郵便番号
                          address: "東京都港区芝...",                                 # 店舗の住所
                          tel: "0220484-258",                                   # 店舗の電話番号
                          email: "vKmUdoTPoZ@pHgl.com",                         # 店舗のメールアドレス
                          external_id: "LbMv4bHsbWCqDTf0zfBZLhIfXkhurUo",       # 店舗の外部ID
...
response = $client.send(Pokepay::Request::CreateUserAccount.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # user_id: ユーザーID
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          name: "iyW36fzpC1LacpgbquK4ctaArasf2C9oXQPtgLc1Set1MDKTvNQ0rCXz0uoFODGAa8SDVM1XYomo34vuRtbhV03WfeVZ33YTMWJ8WyhXVONXfFDEl", # ウォレット名
                          external_id: "GMUV9hM2iR9OpWyx1wSLMII6MVOsuE",        # 外部ID
                          metadata: "{\"key1\":\"foo\",\"key2\":\"bar\"}"       # ウォレットに付加するメタデータ
))
```
...
response = $client.send(Pokepay::Request::GetPrivateMoneyOrganizationSummaries.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          from: "2020-07-27T08:55:32.000000+09:00",             # 開始日時(toと同時に指定する必要有)
                          to: "2023-01-20T14:10:24.000000+09:00",               # 終了日時(fromと同時に指定する必要有)
                          page: 1,                                              # ページ番号
                          per_page: 50                                          # 1ページ分の取引数
))
```
...
response = $client.send(Pokepay::Request::GetPrivateMoneySummary.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          from: "2020-08-27T13:32:42.000000+09:00",             # 開始日時
                          to: "2023-02-05T05:44:27.000000+09:00"                # 終了日時
))
```
...
response = $client.send(Pokepay::Request::BulkCreateTransaction.new(
                          "DLVqvsVLk",                                          # name: 一括取引タスク名
                          "nEaqGuka",                                           # content: 取引する情報のCSV
                          "jXibPkv38K6qEiNFuB5rQfnBPmOdfc4wSZIC",               # request_id: リクエストID
                          description: "swyhwqioJwuAbKC6uMak8fZDWJL4htSMPTogTmpQI4zumDDmEyYmrxmSgrRCqBKkvSvqTi6yhilCacREfqJJF68QQ8SqgfwKLpbDMHQr0Y8jIs9neTN2VuEQ", # 一括取引の説明
                          private_money_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" # マネーID
))
```
...
response = $client.send(Pokepay::Request::CreateCampaign.new(
                          "rulrooc7GI04b5W8PYO1FumWRCEJ7Thnkdp7V6wkITmxtzJEWOVFs1bhzrgf6TKzKR77DlUhU7SCeD9aPBzhOFeS8eodjVkMBmNAKgJA24NRoqZ9pwxeFLShjPFZsPmOzqL7vxf", # name: キャンペーン名
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # private_money_id: マネーID
                          "2021-09-24T12:54:32.000000+09:00",                   # starts_at: キャンペーン開始日時
                          "2022-03-01T21:14:30.000000+09:00",                   # ends_at: キャンペーン終了日時
                          8829,                                                 # priority: キャンペーンの適用優先度
                          "external-transaction",                               # event: イベント種別
                          bear_point_shop_id: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", # ポイント負担先店舗ID
                          description: "GyWR7aDrfV",                            # キャンペーンの説明文
                          status: "enabled",                                    # キャンペーン作成時の状態
                          point_expires_at: "2021-07-12T23:49:07.000000+09:00", # ポイント有効期限(絶対日時指定)
                          point_expires_in_days: 5528,                          # ポイント有効期限(相対日数指定)
  ...
  "to": "23:59"
                          ...
                          minimum_number_for_combination_purchase: 6293,        # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups: false,                  # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount: 6156,                               # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount: 1182,                         # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
  ...
  "value": "male"
                          ...
                          budget_caps_amount: 2102195677                        # キャンペーン予算上限
))
```
...
response = $client.send(Pokepay::Request::ListCampaigns.new(
                          ...
                          available_from: "2021-08-01T22:01:29.000000+09:00",   # 指定された日時以降に適用可能期間が含まれているか
                          available_to: "2021-05-13T22:37:00.000000+09:00",     # 指定された日時以前に適用可能期間が含まれているか
                          page: 1,                                              # ページ番号
...
response = $client.send(Pokepay::Request::UpdateCampaign.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # campaign_id: キャンペーンID
                          name: "LPQhGb2R3CqxMAdPh4olbFrqofOERZYL8sBf25qpYimKMgmGNOXD0RUEvM3Uqt9sZ6bsXRyoZ3CAWMOIXpVzKRUAVJom0fK3dzhIR6DcszwMa7aVrBxw9GLn73ARcr2", # キャンペーン名
                          starts_at: "2023-05-16T21:43:27.000000+09:00",        # キャンペーン開始日時
                          ends_at: "2023-01-23T09:25:01.000000+09:00",          # キャンペーン終了日時
                          priority: 8940,                                       # キャンペーンの適用優先度
                          event: "payment",                                     # イベント種別
                          description: "XobU7DtRhK3enWgRJlYqZID9HUbfyKGew3qWJYHlFM7ukduIbQs8fKW2O2ICgnd3HK7qf63xB9iWuCeRR1YjXNjIooik7xMRjWaQsNZ3YALBYbzno9kg8c9MFxjIv6devgWHmqRlnP8UWQirnIhAntjE0", # キャンペーンの説明文
                          status: "disabled",                                   # キャンペーン作成時の状態
                          point_expires_at: "2021-06-30T07:39:57.000000+09:00", # ポイント有効期限(絶対日時指定)
                          point_expires_in_days: 9073,                          # ポイント有効期限(相対日数指定)
                          is_exclusive: true,                                   # キャンペーンの重複設定
                          subject: "all",                                       # ポイント付与の対象金額の種別
                          amount_based_point_rules: [{
  "point_amount": 5,
  ...
  "required_count": 2
                          ...
                          applicable_days_of_week: [5, 1, 2],                   # キャンペーンを適用する曜日 (複数指定)
                          applicable_time_ranges: [{
  "from": "12:00",
  "to": "23:59"
}, {
  ...
  "to": "23:59"
                          ...
                          minimum_number_for_combination_purchase: 3916,        # 複数種類の商品を同時購入するときの商品種別数の下限
                          exist_in_each_product_groups: false,                  # 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
                          max_point_amount: 8611,                               # キャンペーンによって付与されるポイントの上限
                          max_total_point_amount: 2010,                         # キャンペーンによって付与されるの1人当たりの累計ポイントの上限
                          applicable_account_metadata: {
  "key": "sex",
  "value": "male"
},                                                                              # ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
                          budget_caps_amount: 986409287                         # キャンペーン予算上限
))
```
...
response = $client.send(Pokepay::Request::ListCoupons.new(
                          ...
                          coupon_id: "2",                                       # クーポンID
                          coupon_name: "qwXyr4egl",                             # クーポン名
                          issued_shop_name: "gUtY",                             # 発行店舗名
                          available_shop_name: "GWe",                           # 利用可能店舗名
                          available_from: "2020-05-23T12:52:18.000000+09:00",   # 利用可能期間 (開始日時)
                          available_to: "2022-03-16T01:16:47.000000+09:00",     # 利用可能期間 (終了日時)
                          page: 1,                                              # ページ番号
                          per_page: 50                                          # 1ページ分の取得数
))
```
...
response = $client.send(Pokepay::Request::CreateCoupon.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
                          "VX9Q8gDg0Z",
                          "2022-12-21T16:10:52.000000+09:00",
                          "2021-08-11T02:56:16.000000+09:00",
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # issued_shop_id: 発行元の店舗ID
                          description: "9",
                          discount_amount: 6887,
                          discount_percentage: 987.0,
                          discount_upper_limit: 9346,
                          display_starts_at: "2022-10-18T02:26:24.000000+09:00", # クーポンの掲載期間(開始日時)
                          display_ends_at: "2020-11-12T02:18:30.000000+09:00",  # クーポンの掲載期間(終了日時)
                          is_disabled: true,                                    # 無効化フラグ
                          is_hidden: false,                                     # クーポン一覧に掲載されるかどうか
                          is_public: true,                                      # アプリ配信なしで受け取れるかどうか
                          code: "O",                                            # クーポン受け取りコード
                          usage_limit: 6932,                                    # ユーザごとの利用可能回数(NULLの場合は無制限)
                          min_amount: 4852,                                     # クーポン適用可能な最小取引額
...
response = $client.send(Pokepay::Request::UpdateCoupon.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               # coupon_id: クーポンID
                          name: "LOxAhPND2oQuF8OnlVDHk4s2aQ4HT9Di6YcJiFvywrnjNHmwuaxAdn0FqKANM4L4ufaoHXuVyMAym2Trmuft",
                          description: "dibUSKU2C8oUXHjJYtlC6UOsuQMfqpNYuUUwGHR46mi8cpdI7f238rmvoh4US4o9BvdocRv9DivHaGPJSeMxxYeDhWZuh6F7SUfOcJzzdje0lKFzJL5l8V3G9Jmex2y9UwvO7kgRCm8BZ7KgiXyRLJMaZ3yqPHClk2RjOJcqzJy3InRi1pFAyG9jT0hQHdmZJe",
                          discount_amount: 3217,
                          discount_percentage: 6068.0,
                          discount_upper_limit: 1422,
                          starts_at: "2021-04-13T11:34:34.000000+09:00",
                          ends_at: "2022-02-27T04:10:02.000000+09:00",
                          display_starts_at: "2021-11-23T00:05:17.000000+09:00", # クーポンの掲載期間(開始日時)
                          display_ends_at: "2023-12-17T09:09:34.000000+09:00",  # クーポンの掲載期間(終了日時)
                          is_disabled: true,                                    # 無効化フラグ
                          is_hidden: false,                                     # クーポン一覧に掲載されるかどうか
                          is_public: false,                                     # アプリ配信なしで受け取れるかどうか
                          code: "DrzfTw9na",                                    # クーポン受け取りコード
                          usage_limit: 5287,                                    # ユーザごとの利用可能回数(NULLの場合は無制限)
                          min_amount: 113,                                      # クーポン適用可能な最小取引額
                          is_shop_specified: false,                             # 特定店舗限定のクーポンかどうか
...
response = $client.send(Pokepay::Request::CreateBank.new(
                          ...
                          email: "w5RQGE3O8p@52jo.com",                         # ユーザーのメールアドレス
