...
response = $client.send(Pokepay::Request::CreateTopupTransaction.new(
                          ...
                          money_amount: 4404,                                   // マネー額
                          point_amount: 8636,                                   // ポイント額
...
response = $client.send(Pokepay::Request::ListTransactions.new(
                          from: "2023-08-28T23:24:33.000000+09:00",             // 開始日時
                          to: "2019-04-09T15:47:52.000000+09:00",               // 終了日時
                          page: 1,                                              // ページ番号
...
response = $client.send(Pokepay::Request::CreateCheck.new(
                          ...
                          money_amount: 9741,                                   // 付与マネー額
                          point_amount: 4529,                                   // 付与ポイント額
                          description: "test check",                            // 説明文(アプリ上で取引の説明文として表示される)
                          is_onetime: true,                                     // ワンタイムかどうか。真の場合1度読み込まれた時点でそのチャージQRは失効する(デフォルト値は真)
                          usage_limit: 8348,                                    // ワンタイムでない場合、複数ユーザから読み取られ得る。その場合の最大読み取り回数
                          expires_at: "2020-08-04T00:29:17.000000+09:00",       // チャージQR自体の失効日時
                          point_expires_at: "2023-09-15T13:45:39.000000+09:00", // チャージQRによって付与されるポイントの失効日時
...
response = $client.send(Pokepay::Request::CreateOrganization.new(
                          ...
                          "7pQWxFC8n1@A7if.com",                                // issuer_admin_user_email: 発行体担当者メールアドレス
                          "1ukpIpIK96@X2VT.com",                                // member_admin_user_email: 新規組織担当者メールアドレス
                          bank_name: "XYZ銀行",                                   // 銀行名
...
response = $client.send(Pokepay::Request::CreateShop.new(
                          ...
                          shop_postal_code: "603-6412",                         // 店舗の郵便番号
                          shop_address: "東京都港区芝...",                            // 店舗の住所
                          shop_tel: "0035574",                                  // 店舗の電話番号
                          shop_email: "qnCQWbBzHg@p9wu.com",                    // 店舗のメールアドレス
                          shop_external_id: "GDkO3T",                           // 店舗の外部ID
...
response = $client.send(Pokepay::Request::GetPrivateMoneyOrganizationSummaries.new(
                          "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",               // private_money_id: マネーID
                          from: "2016-07-29T20:06:18.000000+09:00",             // 開始日時(toと同時に指定する必要有)
                          to: "2020-08-06T02:09:17.000000+09:00",               // 終了日時(fromと同時に指定する必要有)
                          page: 1,                                              // ページ番号
                          per_page: 50                                          // 1ページ分の取引数
))
```
